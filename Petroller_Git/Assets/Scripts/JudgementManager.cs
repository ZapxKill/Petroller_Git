        ...
        if (inputSource == currentCommand)
        {
            for (int i = 0; i < judgementTiming.Length; i++)
            {
                if (remainingjudgementTime >= judgementTiming[i])
                {
                    ...
                    {
                        case 0:
                            effectAnimator.Play("Happy");
                            commandAnimator.SetInteger("index", 7);
                            commandImage.transform.localScale = new Vector3(0.6f, 0.6f, 0.6f);
                            commandAnimator.Play("HappyEmo");
                            Invoke("hideCommandUI", 1f);
                            break;
                        case 1:
                            effectAnimator.Play("Happy");
                            hideCommandUI();
                            break;
                        case 2:
                            effectAnimator.Play("Shake");
                            hideCommandUI();
                            break;
                        case 3:
                            effectAnimator.Play("Angry");
                            hideCommandUI();
                            break;
                        default:
                            effectAnimator.Play("Angry");
